...
// Introducción a maldev, self-inject en un proceso
int main(int argc, char** argv)
{
    ...
    // Necesito alocar memoria para el shellcode
    LPVOID allocated_memory = VirtualAlloc(
		NULL,                          // Let the system determine the address. Este parametro es NULL porque no queremos especificar una dirección
	...
	// Ahora necesito copiar el shellcode a la memoria alocada
	RtlCopyMemory( // Utilizo esta función rara en vez de memcpy porque es parte de la API de Windows, pero en el fondo es lo mismo
    ...
    // Check if the thread handle is valid before calling WaitForSingleObject
    if (thread == NULL) {
